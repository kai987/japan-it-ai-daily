...
const languagePatterns = {
  ...
  ja: {
    ...
    limitation: /(ただし|一方で|対応済みですが|(?:リスク|Risk)がある|制約|限界|注意|保証|とは限ら|未対応|未提供|Preview|プレビュー|Alpha|アルファ|参考値|単発|一度|一回|条件|依存|課題|反例|追加確認|一般化|本番|実Workload|実Application|自己申告.{0,30}(?:分け|区別|照合)|(?:ため|ので|なら|場合).{0,60}(?:必要|必須))/i,
  },
};
