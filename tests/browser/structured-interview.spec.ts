...
for (const date of structuredInterviewDates().slice(-3)) {
  const pilot = readStructuredInterview(date)!;
  for (const locale of ['zh', 'ja'] as const) {
    for (const width of [1440, 390]) {
      test(`${date}/${locale} structured interview renders unchanged content at ${width}px`, async ({ page }, info) => {
        // Mirror an explicit user language choice; the site's fresh-session default is Japanese.
        ...
        const day = page.locator('.interview-day').filter({ has: page.locator(`a.daily-link[href$="/daily/${date}/"]`) });
        ...
        await page.screenshot({ path: info.outputPath(`structured-interview-${date}-${locale}-${width}.png`) });
